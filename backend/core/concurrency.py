...
class TaskQueue:
    ...
    async def start(self):
        ...
        system_logger.info(f"【任务队列】已启动，最大并发: {self.max_concurrent}")
    ...
    async def stop(self):
        ...
        for task_id, task in list(self.active_tasks.items()):
            task.cancel()
            system_logger.info(f"【任务队列】取消任务: {task_id}")
        ...
        system_logger.info("【任务队列】已停止")
    
    async def submit(self, 
                     coro: Callable, 
                     session_id: str,
                     user_id: Optional[str] = None,
                     priority: int = 5) -> asyncio.Future:
        """
        提交任务到队列
        ...
        # 放入优先级队列
        await self.queue.put((priority, time.time(), queued_task))
        system_logger.info(
            f"【任务队列】任务 {task_id[:8]}... 已提交，"
            f"队列长度: {self.queue.qsize()}，优先级: {priority}"
        )
            ...
            try:
                ...
                if wait_time > task.timeout:
                    system_logger.warning(
                        f"【任务队列】任务 {task.task_id[:8]}... 已超时，跳过"
                    )
                    task.future.set_exception(TimeoutError("任务等待超时"))
                ...
                async with self._semaphore:
                    # 获取GPU锁
                    if not await self.gpu_lock.acquire(task.task_id, timeout=60.0):
                        task.future.set_exception(TimeoutError("获取GPU资源超时"))
                        self.queue.task_done()
                        continue
                    
                    try:
                        # 执行任务
                        system_logger.info(f"【任务队列】开始执行任务 {task.task_id[:8]}...")
                        ...
                        system_logger.info(
                            f"【任务队列】任务 {task.task_id[:8]}... 完成，耗时: {duration:.2f}s"
                        )
                        ...
                        system_logger.error(
                            f"【任务队列】任务 {task.task_id[:8]}... 执行失败: {e}"
                        )
                        if not task.future.done():
                ...
                system_logger.error(f"【任务队列】处理任务时出错: {e}")
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "gpu_lock": self.gpu_lock.get_status()
        }
